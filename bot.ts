...
let bot: Client = new Client(process.env.d_TOKEN, {
...
bot.on("messageCreate", () => { // When a message is created
    ...
    //             case "wtf":
    //                 if (channelID === "735315394151055491") { //just to show how 2 post images so limited to #test
    //                     bot.sendChannelTyping(channelID);
    //                     const body = fs.readFileSync("./images/headpat1.png");
    //                     bot.createMessage(channelID, undefined, { file: body, name: "headpat1.png" });
    //                 }
    //                 break;
    ...
    //                     if (cmds[newCmd] === undefined) { //dont add command if it exists already
    ...
    //                     let newCmd = args.shift();
    //                     if (cmds[newCmd] !== undefined) { //make sure command exists
    //                         cmds[newCmd] = args.join(" ");
